...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programmerID ? 'gold' : 'crimson', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the programmer id state to be dev.id */ setProgrammerID(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          programmerID
            ? `🎉 Let's celebrate ${getNameOfFeatured(programmers, programmerID)}! 🥳`
